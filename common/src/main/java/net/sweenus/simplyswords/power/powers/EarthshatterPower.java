...
import net.sweenus.simplyswords.world.EarthshatterSunderManager;

import java.util.List;

public class EarthshatterPower extends RunefusedGemPower {

    public EarthshatterPower() {
        super(false);
    }
    ...
    public void postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        ...
        if (attacker.getRandom().nextInt(100) >= Config.gemPowers.earthshatter.chance) {
            return;
        }
    ...
    public void appendTooltip(ItemStack itemStack, Item.TooltipContext tooltipContext, List<Text> tooltip, TooltipType type, boolean isRunic) {
        if (isRunic) {
            tooltip.add(Text.translatable("item.simplyswords.earthshattersworditem.tooltip1").setStyle(Styles.RUNIC));
        } else {
            tooltip.add(Text.translatable("item.simplyswords.uniquesworditem.runefused_power.earthshatter").setStyle(Styles.RUNIC));
        }

        if (TooltipUtils.shouldDisplayTooltip(itemStack, null)) {
            tooltip.add(Text.literal("").append(Text.translatable("item.simplyswords.earthshattersworditem.tooltip2")).setStyle(Styles.RUNIC_DESCRIPTION));
        }
    }
    ...
    public static class Settings extends TooltipSettings {
        ...
        public Settings() {
            super(GemPowerRegistry.EARTHSHATTER);
        }
