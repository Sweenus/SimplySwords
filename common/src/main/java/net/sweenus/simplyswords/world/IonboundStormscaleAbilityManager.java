...
public final class IonboundStormscaleAbilityManager {
    ...
    private static final UUID BEAM_MOVEMENT_SLOW_ID = UUID.fromString("7f2c1a94-6d3b-4a58-9f0e-2c5b8d61a3e7");
    private static final String BEAM_MOVEMENT_SLOW_NAME = "simplyswords_ion_beam_channel_slow";
    ...
    public static IonCubeComponent getCubes(ItemStack stack) {
        return stack == null || stack.isEmpty()
                ...
                : ComponentTypeRegistry.ION_CUBES.getOrDefault(stack, IonCubeComponent.DEFAULT);
    }
    ...
    public static void tickStack(ItemStack stack, World world, Entity entity) {
        ...
        if (cubes.cubes() >= IonCubeComponent.MAX_CUBES) {
            ...
            if (state.rechargeTicks >= Math.max(1, Config.uniqueEffects.ionbound_stormscale.rechargeInterval)) {
                ...
                state.rechargeTicks = 0;
                ComponentTypeRegistry.ION_CUBES.set(stack, cubes);
    ...
    private static boolean consumeCube(ItemStack stack) {
        ...
        ComponentTypeRegistry.ION_CUBES.set(stack, cubes.consume());
        return true;
    }
    ...
    private static void applyBeamMovementSlow(LivingEntity actor) {
        ...
        if (current != null && current.getValue() == reduction
                && current.getOperation() == EntityAttributeModifier.Operation.MULTIPLY_TOTAL) {
            return;
        }
        movementSpeed.removeModifier(BEAM_MOVEMENT_SLOW_ID);
        movementSpeed.addTemporaryModifier(new EntityAttributeModifier(BEAM_MOVEMENT_SLOW_ID,
                BEAM_MOVEMENT_SLOW_NAME, reduction,
    ...
    private static void checkpointRecharge(RechargeState state) {
        ...
        if (!checkpoint.equals(current)) {
            ComponentTypeRegistry.ION_CUBES.set(state.stackReference, checkpoint);
        }
    }
