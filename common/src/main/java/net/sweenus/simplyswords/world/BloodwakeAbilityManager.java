...
public final class BloodwakeAbilityManager {
    ...
    public static int getFrenzy(ItemStack stack) {
        return stack == null || stack.isEmpty() ? 0
                : MathHelper.clamp(ComponentTypeRegistry.STORED_CHARGE.getOrDefault(stack, StoredChargeComponent.DEFAULT).charge(), 0, MAX_FRENZY);
    }

    public static void setFrenzy(ItemStack stack, int amount) {
        if (stack != null && !stack.isEmpty()) {
            ComponentTypeRegistry.STORED_CHARGE.set(stack, new StoredChargeComponent(MathHelper.clamp(amount, 0, MAX_FRENZY)));
        }
    }
    ...
    private static boolean isDirectAttack(DamageSource source) {
        ItemStack weapon = source.getAttacker() instanceof LivingEntity attacker
        ...
        return source.isOf(DamageTypes.PLAYER_ATTACK)
                ...
                || !source.isIndirect() && !"indirectMagic".equals(source.getName()) && !"magic".equals(source.getName());
    }
