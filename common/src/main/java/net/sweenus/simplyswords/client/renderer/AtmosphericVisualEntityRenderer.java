...
public class AtmosphericVisualEntityRenderer extends EntityRenderer<AtmosphericVisualEntity> {
    ...
    private static final Identifier CRYSTAL_SPRITE = Identifier.ofVanilla("block/smooth_basalt");
    ...
    private static void crystalVertex(VertexConsumer vertices, MatrixStack.Entry entry, Vec3d point,
        ...
        vertices.vertex(entry, (float) point.x, (float) point.y, (float) point.z)
                .color(shade, shade, shade * 1.06F, alpha)
                .texture(u, v)
                .overlay(OverlayTexture.DEFAULT_UV)
                .light(light)
                .normal(entry, (float) normal.x * sign, (float) normal.y * sign, (float) normal.z * sign);
    }
    ...
    private static void vertex(VertexConsumer vertices, Matrix4f matrix, float x, float y, float z,
        ...
        vertices.vertex(matrix, x, y, z).color(r, g, b, a).texture(u, v)
                .overlay(OverlayTexture.DEFAULT_UV).light(light).normal(0.0F, 1.0F, 0.0F);
    }
    ...
    private static Identifier id(String name) { return Identifier.of(SimplySwords.MOD_ID, "textures/effect/storm/" + name); }
