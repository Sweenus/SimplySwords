package net.sweenus.simplyswords.client.hud;
 ...
 * bar, and the Soulstealer / Stormbringer charge pips.
 * <p>
 * Registered against {@code ClientGuiEvent.RENDER_HUD} rather than mixed into
 * {@code InGameHud.render}, because Forge 1.20.1 swaps in {@code ForgeGui}, which overrides
 * {@code render} without calling {@code super} - a mixin there never runs on Forge.
 */
public final class WeaponHudRenderer {
    ...
    private static float lastHeatRenderTime;
    private static boolean heatDisplayInitialized;
    private static float breachTint;
    private static float lastBreachRenderTime;

    private WeaponHudRenderer() {
    }

    public static void render(DrawContext context, float tickDelta) {
        renderAstralBreachTint(context, tickDelta);
        renderWeaponHud(context, tickDelta);
    }

    private static void renderAstralBreachTint(DrawContext context, float tickDelta) {
        MinecraftClient client = MinecraftClient.getInstance();
        if (client.world == null || !Config.general.enableModernFieldEffects) {
            breachTint = 0.0F;
            lastBreachRenderTime = 0.0F;
            return;
        }
        ...
        float elapsed = lastBreachRenderTime == 0.0F
                ? 1.0F : MathHelper.clamp(renderTime - lastBreachRenderTime, 0.0F, 5.0F);
        float smoothing = 1.0F - (float) Math.exp(-0.28F * elapsed);
        breachTint += (target - breachTint) * smoothing;
        lastBreachRenderTime = renderTime;
        if (breachTint < 0.005F) {
            return;
        }
    ...
    private static void renderWeaponHud(DrawContext context, float tickDelta) {
        MinecraftClient client = MinecraftClient.getInstance();
        ...
        ItemStack moltenEdge = selectMoltenEdge(client);
    ...
    private static void renderHeatBar(DrawContext context, MinecraftClient client, float tickDelta, MoltenHeatComponent component) {
        long worldTime = client.world == null ? 0L : client.world.getTime();
        int ventDrain = Math.max(1, Config.uniqueEffects.molten_edge.ventDrainPerTick);
        float targetHeat = component.heatAt(worldTime, ventDrain);
        float renderTime = worldTime + tickDelta;
        if (!heatDisplayInitialized) {
            displayedHeat = targetHeat;
            lastHeatRenderTime = renderTime;
            heatDisplayInitialized = true;
        } else {
            float elapsedTicks = renderTime - lastHeatRenderTime;
            if (elapsedTicks < 0.0F || elapsedTicks > 5.0F) {
                displayedHeat = targetHeat;
            } else {
                float smoothing = 1.0F - (float) Math.exp(-HEAT_SMOOTHING_RATE * elapsedTicks);
                displayedHeat += (targetHeat - displayedHeat) * smoothing;
                if (Math.abs(targetHeat - displayedHeat) < 0.01F) {
                    displayedHeat = targetHeat;
                }
            }
            lastHeatRenderTime = renderTime;
        ...
        float fraction = MathHelper.clamp(displayedHeat / MoltenHeatComponent.MAX_HEAT, 0.0F, 1.0F);
        ...
        pushWeaponHudTransform(context);
        if (danger > 0.0F) {
            int glowAlpha = MathHelper.clamp((int) ((28.0F + 74.0F * pulse) * danger), 0, 120);
