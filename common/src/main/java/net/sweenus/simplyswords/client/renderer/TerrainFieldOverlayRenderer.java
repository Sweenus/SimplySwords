...
public final class TerrainFieldOverlayRenderer {
    ...
    private void renderFootprint(World world, UUID id,
        ...
        VertexConsumer opaqueTexturedVertices = null;
        VertexConsumer translucentTexturedVertices = null;
        for (PreparedFace prepared : preparedFaces) {
            ...
            if (faceOpacity < 0.999F) {
                if (translucentTexturedVertices == null) {
                    translucentTexturedVertices = vertexConsumers.getBuffer(
                            RenderLayer.getEntityTranslucent(SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE));
                }
                texturedVertices = translucentTexturedVertices;
            } else {
                if (opaqueTexturedVertices == null) {
                    opaqueTexturedVertices = vertexConsumers.getBuffer(RenderLayer.getCutout());
                }
                texturedVertices = opaqueTexturedVertices;
            }
            ...
            for (SplatterPart part : shape.parts) {
                for (FluidSurface surface : cache.fluidSurfaces) {
                    drawFluidSurface(
                            ...
                            matrices, opaqueTexturedVertices, sprites,
    ...
    private static void putGloamEyeColorVertex(VertexConsumer vertices, MatrixStack.Entry matrices,
        ...
        vertices.vertex(matrices.getPositionMatrix(), (float) point.x, (float) point.y, (float) point.z)
    ...
    private static void putGloamEyeGlowVertex(VertexConsumer vertices, MatrixStack.Entry matrices,
        ...
        vertices.vertex(matrices.getPositionMatrix(), (float) point.x, (float) point.y, (float) point.z)
                ...
                .normal(matrices.getNormalMatrix(), 0.0F, 1.0F, 0.0F).next();
    }
    ...
    private static void putBloodPatchVertex(
        ...
        var vertex = vertices.vertex(
                        matrices.getPositionMatrix(), (float) localX, (float) localY, (float) localZ)
                .color(point.tint.red, point.tint.green, point.tint.blue,
                        MathHelper.clamp(Math.round(opacity * 255.0F), 0, 255))
                .texture(normalizedSpriteU(texture.sprite, uv.u),
                        normalizedSpriteV(texture.sprite, uv.v));
        if (opacity < 0.999F) {
            vertex.overlay(OverlayTexture.DEFAULT_UV);
        }
        vertex.light(light).normal(
                matrices.getNormalMatrix(), direction.getOffsetX(),
    ...
    private static void putTextureVertex(
        ...
        var vertex = vertices.vertex(matrices.getPositionMatrix(), (float) x, y, (float) z)
    ...
    static double normalizedFromSpriteU(Sprite sprite, float atlasU) {
        return sprite.getFrameFromU(atlasU) / 16.0;
    }

    static double normalizedFromSpriteV(Sprite sprite, float atlasV) {
        return sprite.getFrameFromV(atlasV) / 16.0;
    }
    ...
    private record FaceTextureMapping(
        ...
        private static FaceTextureMapping from(
            ...
            for (int i = 0; i < vertices.length; i++) {
                ...
                double u = normalizedFromSpriteU(sprite, Float.intBitsToFloat(data[offset + 4]));
                double v = normalizedFromSpriteV(sprite, Float.intBitsToFloat(data[offset + 5]));
