...
public final class MartialCommandEldritchMasteryTuning implements MasteryTuningSnapshot {
    public enum Setting {
    ...
    public static final MartialCommandEldritchMasteryTuning EMPTY = new MartialCommandEldritchMasteryTuning(Map.of());
    private final EnumMap<Setting, Double> values;

    private MartialCommandEldritchMasteryTuning(Map<Setting, Double> values) {
    ...
    public MartialCommandEldritchMasteryTuning with(Setting setting, double value) {
        EnumMap<Setting, Double> updated = new EnumMap<>(values);
        updated.put(setting, setting.validate(value));
        return new MartialCommandEldritchMasteryTuning(updated);
    }

    public MartialCommandEldritchMasteryTuning add(Setting setting, double amount, double fallback) {
        return with(setting, get(setting, fallback) + amount);
    }

    public MartialCommandEldritchMasteryTuning multiply(Setting setting, double factor, double fallback) {
