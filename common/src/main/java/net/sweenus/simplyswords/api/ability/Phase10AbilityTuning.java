...
public final class Phase10AbilityTuning {
    public enum Setting {
    ...
    public static final Phase10AbilityTuning EMPTY = new Phase10AbilityTuning(Map.of());
    private final EnumMap<Setting, Double> values;

    private Phase10AbilityTuning(Map<Setting, Double> values) {
    ...
    public Phase10AbilityTuning with(Setting setting, double value) {
        EnumMap<Setting, Double> updated = new EnumMap<>(values);
        updated.put(setting, setting.validate(value));
        return new Phase10AbilityTuning(updated);
    }

    public Phase10AbilityTuning add(Setting setting, double amount, double fallback) {
        return with(setting, get(setting, fallback) + amount);
    }

    public Phase10AbilityTuning multiply(Setting setting, double factor, double fallback) {
